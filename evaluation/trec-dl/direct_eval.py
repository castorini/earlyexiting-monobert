...
def get_args():
    ...
    parser.add_argument("-r", "--reference", required=False, default="2019qrels-pass.txt",
                        help="Qrel file")
...
def eval_run_file(args, run_fname):
    ...
    if not os.path.exists(args.reference):
        raise FileNotFoundError("Make sure to copy/link the qrels file to the current directory.")
...
def save_to_np(args, metrics, avg_layer):
    ...
    value = (avg_layer, metrics["ndcg_cut_100"])
    record[key] = value
